...
function riskManager(results, predictedCategory) {
  let riskFactor = results.filter(r => convertToBigSmall(r.number) === predictedCategory).length;
  return riskFactor > 5 ? predictedCategory : (predictedCategory === "Big" ? "Small" : "Big");
}
...
// AI-6: Dynamic Adjustment AI (🔥 Upgraded)
...
// AI-7: Deep Pattern AI (🔥 Intelligent Pattern Recognition)
async function neuralPatternAI() {
  return new Promise((resolve) => {
    ...
    onValue(resultsRef, (snapshot) => {
      ...
      // Deep Wave (Big x3 → Small x3 → Big x3)
